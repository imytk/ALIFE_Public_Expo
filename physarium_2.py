...
def calculate_food_map_threaded(food_sources, width, height):
    """Calculate food map in separate thread"""
    food_map = np.zeros((height, width), dtype=np.float32)
    ...
    for food_x, food_y in food_sources:
        ...
        x_min = max(0, int(food_x - radius))
        x_max = min(width, int(food_x + radius + 1))
        y_min = max(0, int(food_y - radius))
        y_max = min(height, int(food_y + radius + 1))
        ...
        for y in range(y_min, y_max):
            ...
            for x in range(x_min, x_max):
                ...
                if dist_sq <= radius_sq:
                    ...
                    value = strength * max(0, 1 - distance / radius)
                    food_map[y, x] = max(food_map[y, x], value)
    
    return food_map

def update_agents_batch(agents_batch, trail_map, food_map, params):
    """Update a batch of agents in parallel"""
    for agent in agents_batch:
        agent.update(trail_map, food_map, params)
        agent.deposit_trail(trail_map, params['deposit_amount'])
...
class PhysarumSimulation:
    def __init__(self, width=800, height=600):
        ...
        pygame.display.set_caption("Multithreaded Physarum - Complexity from Simplicity")
        self.clock = pygame.time.Clock()
        
        # Create maps
        self.trail_map = np.zeros((height, width), dtype=np.float32)
        self.food_map = np.zeros((height, width), dtype=np.float32)
        self.food_map_buffer = np.zeros((height, width), dtype=np.float32)  # Buffer for threading
        ...
        self.food_calculation_future = None
        self.food_queue = queue.Queue()
        self.food_update_pending = False
    ...
    def add_demo_food(self):
        ...
        # Trigger initial food map calculation
        self.request_food_map_update()
    
    def request_food_map_update(self):
        """Request food map update in background thread"""
        if not self.food_update_pending:
            self.food_update_pending = True
            # Submit food map calculation to thread pool
            self.food_calculation_future = self.thread_pool.submit(
                calculate_food_map_threaded, 
                list(self.food_sources), 
                self.width, 
                self.height
            )
    
    def check_food_map_update(self):
        """Check if food map calculation is complete and update if ready"""
        if self.food_calculation_future and self.food_calculation_future.done():
            try:
                # Get the calculated food map
                new_food_map = self.food_calculation_future.result()
                self.food_map = new_food_map
                self.food_update_pending = False
                self.food_calculation_future = None
            except Exception as e:
                print(f"Food map calculation error: {e}")
                self.food_update_pending = False
    ...
    def draw_simulation(self):
        ...
        threading_status = "MT" if self.params['num_threads'] > 1 else "ST"
        if self.food_update_pending:
            threading_status += " (Food Calc)"
        
        # Draw enhanced UI
        self.draw_ui(threading_status)
        
        pygame.display.flip()
    
    def draw_ui(self, threading_status=""):
        """Draw enhanced user interface with threading info"""
        ...
        main_text = f"FPS: {self.fps:.1f} | Agents: {len(self.agents)} | Food: {len(self.food_sources)} | {status} | {threading_status}"
        ...
        controls = [
            ...
            "+/-: Agents | TAB: Colors | Multithreaded for Performance!"
        ]
...
class EnhancedControlPanel:
    def __init__(self, simulation):
        self.simulation = simulation
        self.root = tk.Tk()
        self.root.title("Multithreaded Physarum - Complexity from Simplicity")
    ...
    def create_widgets(self):
        ...
        subtitle_label = ttk.Label(main_frame, text="Multithreaded Physarum Simulation", 
                                  font=('Arial', 10))
        subtitle_label.pack(pady=(0, 15))
        
        # Performance controls
        ttk.Label(main_frame, text="Performance:", font=('Arial', 10, 'bold')).pack(anchor=tk.W)
        
        # Number of threads
        ttk.Label(main_frame, text="Threads:").pack(anchor=tk.W)
        ...
        threads_scale = ttk.Scale(main_frame, from_=1, to=8, variable=self.threads_var,
                                 command=self.update_threads, orient=tk.HORIZONTAL)
        threads_scale.pack(fill=tk.X, pady=(0, 10))
...
• Food calculations run in background
• Agent updates use parallel processing
• Smooth FPS even with many agents

How it works:
• Each agent: Sense → Turn → Deposit
• Simple rules → Complex networks!
• Perfect emergence demonstration

Controls:
• Adjust threads for your CPU
• Mouse: Place/remove food
• Watch networks form instantly!
        """
    ...
    def update_threads(self, value):
        self.simulation.params['num_threads'] = int(float(value))
    ...
    def clear_food(self):
        ...
        self.simulation.request_food_map_update()
...
def run_enhanced_physarum():
    print("Starting Multithreaded Physarum Simulation")
    print("='Complexity from Simplicity' Demo=")
    print("\n🚀 PERFORMANCE FEATURES:")
    print("• Background food map calculations")
    print("• Parallel agent processing")
    print("• Smooth FPS with large populations")
    print("• No frame drops when placing food!")
    ...
    print("  Adjust thread count in control panel")
